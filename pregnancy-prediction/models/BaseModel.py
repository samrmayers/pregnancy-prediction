...
class BaseModel(object):
    ...
    def add_loss_op(self):
        ...
        if self.FLAGS.loss == 'max_margin':
            self.loss = tf.nn.relu(0.6 - self.probabilities) ** 2 * self.label_placeholder + tf.nn.relu(
                self.probabilities) * (1 - self.label_placeholder)
    ...
    def train_on_dataset(self):
        ...
        train_files = [join(self.FLAGS.training_data_dir, f) for f in listdir(self.FLAGS.training_data_dir) if
                       isfile(join(self.FLAGS.training_data_dir, f))]
        dataset = tf.data.Dataset.from_tensor_slices(train_files)
        dataset = dataset.shuffle(buffer_size=len(train_files))
        dataset = dataset.map(
            lambda filename: tuple(tf.numpy_function(self.load_data_npy, [filename, True], [tf.float64, tf.float64])),
        ...
        while self.FLAGS.num_epochs == 0 or epoch < self.FLAGS.num_epochs:
            ...
            toc_all = time.time()
            logging.info(
                'Epoch %d took %.2f seconds: %.2f seconds for loading data' % (epoch, toc_all - tic_all, data_time))
            epoch += 1
    ...
    def train_a_file(self, X_train, Y_train, batch_size, X_dev, Y_dev, epoch):
        ...
        for minibatch_start in np.arange(0, data_size, batch_size):
            ...
            if global_step % self.FLAGS.eval_every == 0:
                ...
                dev_loss = self.sess.run(self.loss,
                                         feed_dict={self.input_placeholder: X_dev, self.label_placeholder: Y_dev})
    ...
    def run(self):
        ...
        if self.FLAGS.mode == 'train':
            ...
            print("For whole test set: auc %f 1-cycle P_preg (%f, %f) 6-cycle P_preg (%f, %f)" % (
                eval_test_metric(X_test, Y_test)))
